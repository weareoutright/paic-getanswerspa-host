...
const getFileNodeMetaBySourceUrl = sourceUrl => {
  const fileNodesMetaByUrls = (0, _store.getStore)().getState().imageNodes.nodeMetaByUrl;
...
const getFileNodeByMediaItemNode = async ({
  ...
  existingNodeMeta && existingNodeMeta.id &&
...
const createLocalFileNode = async ({
  ...
  (_pluginOptions$type = pluginOptions.type) === null || _pluginOptions$type === void 0 ? void 0 : (_pluginOptions$type$M = _pluginOptions$type.MediaItem) === null || _pluginOptions$type$M === void 0 ? void 0 : _pluginOptions$type$M.localFile;

  // if this file is larger than maxFileSizeBytes, don't fetch the remote file
  if (fileSize > maxFileSizeBytes) {
    (0, _store.getStore)().dispatch.postBuildWarningCounts.incrementMaxFileSizeBytesExceeded();
    return null;
  }

  // if this type of file is excluded, don't fetch the remote file
  if (excludeByMimeTypes.includes(mimeType)) {
    (0, _store.getStore)().dispatch.postBuildWarningCounts.incrementMimeTypeExceeded();
    return null;
  }
  ...
  if (hardCacheMediaFiles) {
    try {
      ...
      await _fsExtra.default.ensureDir(_path.default.dirname(hardCachedFilePath));
