...
const checkForNodeUpdates = async ({
  ...
  (0, _store.getStore)().dispatch.develop.pauseRefreshPolling();

  // get the last sourced time
  const lastCompletedSourceTime = await cache.get((0, _store.withPluginKey)(_constants.LAST_COMPLETED_SOURCE_TIME));
  ...
  if (newActions.length) {
    ...
    await cache.set((0, _store.withPluginKey)(_constants.LAST_COMPLETED_SOURCE_TIME), Date.now());
    (0, _store.getStore)().dispatch.develop.resumeRefreshPolling();
  }
};
const refetcher = async (msRefetchInterval, helpers, {
  ...
  try {
    ...
    } = (0, _store.getStore)().getState().develop;
...
const startPollingForContentUpdates = helpers => {
  ...
  const {
    verbose,
    develop
  } = (0, _store.getStore)().getState().gatsbyApi.pluginOptions;
  const restoreContext = (0, _store.snapshotContext)();
