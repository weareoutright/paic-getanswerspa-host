...
const handleCustomScalars = field => {
  ...
  const fieldTypeOfTypeIsACustomScalar = ((_field$type$ofType = field.type.ofType) === null || _field$type$ofType === void 0 ? void 0 : _field$type$ofType.kind) === `SCALAR` && !(0, _helpers.typeIsASupportedScalar)(field.type);
...
const transformFields = ({
  ...
  const transformedFields = fields.reduce((fieldsObject, field) => {
    ...
    const {
      transform,
      description
    } = peek === false ? (0, _fieldTransformers.getFieldTransformers)().find(({
      test
    }) => test(field)) || {} : {};
    if (transform && typeof transform === `function` && peek === false) {
      const transformerApi = {
        ...
        gatsbyNodeTypes,
        description,
        pluginOptions: (0, _getGatsbyApi.getPluginOptions)()
      };
